...
class NewShader extends Phaser.Renderer.WebGL.Pipelines.TextureTintPipeline {
    constructor(game) {
        super({
            game: game,
            renderer: game.renderer,
            ...
            fragShader: `
                ...
                void main(){
                    vec4 texture = texture2D(uMainSampler, outTexCoord); //Create texture variable, containing colour and alpha values of the pixels                  
                    //float color = dot(texture.rgb, vec3(1.0, 1.0, 1.0));
                    float color = texture.rgb * vec3(0.5, 0.5, 0.5);
                    gl_FragColor = vec4(vec3(color), 1.0); //Apply the vec4 to the shader. This effectively ends the shader.
                }
            `
        });
    }
